...
describe.skip("Create car", () => {
  ...
  beforeEach(async () => {
    const signupResponse = await client.post("auth/signup", userData);
  ...
  test("Should be able to create a Car", async () => {
    const carBrandsResponse = await client.get("cars/brands");
    const brand = carBrandsResponse.data.data[0];

    const carModelsResponse = await client.get("cars/models");
    const model = carModelsResponse.data.data.find(
    ...
    // const carByIdResponse = await client.get(`cars/${createdCar.id}`);
    // expect(carByIdResponse.status).toBe(200);
    // expect(carByIdResponse.data.data).toEqual(expectedData);

    // expect(createdCar.carCreatedAt).toBe(
    //   carByIdResponse.data.data.carCreatedAt
    // );
    // expect(createdCar.updatedMileageAt).toBe(carByIdResponse.data.data.updatedMileageAt)

    // expect(
    //   moment(beforeCarCreatedTime).diff(
    //     moment(createdCar.carCreatedAt),
    //     "minute"
    //   )
    // ).toBeLessThanOrEqual(1);
    // expect(
    //   moment(beforeCarCreatedTime).diff(
    //     moment(createdCar.updatedMileageAt),
    //     "minute"
    //   )
    // ).toBeLessThanOrEqual(1);
  });
